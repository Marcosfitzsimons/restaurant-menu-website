...
async function getCategoryData() {
  const res = await fetch(`${siteConfig.endPoints.getCategory.url}Dulces`)
  if (!res.ok) {
...
const DulcesPage = async () => {
  const data = await getCategoryData()
  const productsData: Product[] = data.data

  return (
    <CategoryWrapper title="Dulces" placeholder="ej. Medialunas de manteca">
      {" "}

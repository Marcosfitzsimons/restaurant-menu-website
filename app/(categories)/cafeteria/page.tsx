...
async function getCategoryData() {
  const res = await fetch(`${siteConfig.endPoints.getCategory.url}Cafeteria`)
  if (!res.ok) {
...
const CafeteriaPage = async () => {
  const data = await getCategoryData()
  const productsData: Product[] = data.data

  return (
    <CategoryWrapper title="Cafetería" placeholder="ej. Café cortado">
